...
dds <- DESeqDataSetFromMatrix(countData = counts_matrix,
                              ...
                              design = ~ condition)
dds <- DESeq(dds)
res <- results(dds, contrast = c("condition", "Adipocyte-WT", "FRC-WT"))

# Adjusting for multiple testing, for example, using Benjamini-Hochberg procedure
results <- res[order(res$padj),] # Ordering by adjusted p-value
...
# Assuming 'res' contains the results from DESeq2
...
significant_genes <- res_df[res_df$padj < 0.05 & abs(res_df$log2FoldChange) > 2 & res_df$baseMean > 100, ] %>% unique() %>% arrange(desc(log2FoldChange)) 

# Filter significantly upregulated genes & log fold change in normalized counts (expression) > 2 and rank
gene_Upregulated <- significant_genes[significant_genes$log2FoldChange > 2, ]
nrow((gene_Upregulated))
head(gene_Upregulated)
# [1] 828

# Filter significantly downregulated genes & log fold change in normalized counts (expression) < -2 and rank based on absolute value of log2FoldChange
